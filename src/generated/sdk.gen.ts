...
/**
 * List contacts
 * Returns contacts in the API key scope, newest first. Requires read_only. Search matches name, email or company. Tag filters use any match. Account and platform filters use contact channels. filters.tags always covers the unfiltered in-scope set. Reads use the plan per-minute budget.
 */
 ...
 * Requires read_write. Limited to 20 imports/hour/user. Runs as one mutation. Processes 1–1000 rows with individual errors; duplicate channel or profile email skips the row and merges only tags. Remaining rows fail once the contact cap is reached. platform is ignored with accountId and rejected without it. Envelope profile/account errors fail the request.
 */
export const bulkCreateContacts = <ThrowOnError extends boolean = false>(options: Options<BulkCreateContactsData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).post<BulkCreateContactsResponses, BulkCreateContactsErrors, ThrowOnError>({
        security: [
            {
                scheme: 'bearer',
                type: 'http'
...
/**
 * Delete a contact
 * Requires read_write. Limited to 300 deletes/hour/user. Atomically deletes the contact and all its channels (maximum 20). Unknown or out-of-scope ids return 404 contactNotFound.
 */
export const deleteContact = <ThrowOnError extends boolean = false>(options: Options<DeleteContactData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).delete<DeleteContactResponses, DeleteContactErrors, ThrowOnError>({
        security: [
            {
                scheme: 'bearer',
                type: 'http'
...
/**
 * Get a contact
 * Requires read_only. Returns the contact with distinct conversationIds and channels, oldest first. Unknown or out-of-scope ids return 404 contactNotFound. Counters start when Contacts ships; history is not backfilled.
 */
...
/**
 * Update a contact
 * Requires read_write. Limited to 600 updates/hour/user. Supports partial updates, null clears for email, company, avatarUrl, notes and customFields, and [] to clear tags. customFields replaces the entire object. profileId and channel fields cannot be changed. Unknown or out-of-scope ids return 404 contactNotFound.
 */
export const updateContact = <ThrowOnError extends boolean = false>(options: Options<UpdateContactData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).patch<UpdateContactResponses, UpdateContactErrors, ThrowOnError>({
        security: [
            {
                scheme: 'bearer',
                type: 'http'
 ...
 * Requires read_only. Returns all channels oldest first by createdAt, including lastActiveAt. Unknown or out-of-scope contacts return 404 contactNotFound.
 */
export const getContactChannels = <ThrowOnError extends boolean = false>(options: Options<GetContactChannelsData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetContactChannelsResponses, GetContactChannelsErrors, ThrowOnError>({
        security: [
            {
                scheme: 'bearer',
                type: 'http'
